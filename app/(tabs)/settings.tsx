...
import { DEFAULT_LIST_PAGE_SIZE, clearPlaybackLists, clearPosterCache, clearSearches, clearWatchHistory, getCategoryClassicPageSize, getCategoryPageMode, getLocalCacheSummary, MAX_CUSTOM_SOURCES, saveCategoryClassicPageSize, saveCategoryPageMode, type CategoryClassicPageSize, type CategoryPageMode, type SavedMacCmsSource } from "@/lib/vod-storage";
...
export default function SettingsScreen() {
  ...
  const customSourceCount = sources.filter((source) => source.sourceType !== "official").length;
  const customSourceLimitReached = customSourceCount >= MAX_CUSTOM_SOURCES;
  ...
  const runSelectedCacheClear = async () => {
    ...
    const tasks = selectedClearTargets.map((target) => {
      ...
      return Platform.OS === "android" ? Promise.resolve().then(() => clearVideoCacheAsync()) : Promise.resolve();
    });
    ...
    <ScreenContainer containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          <View style={styles.sourceHeading}><Text style={styles.sectionTitle}>数据源管理</Text><View style={styles.sourceHeaderActions}><Text style={styles.sourceCount}>普通 {customSourceCount}/{MAX_CUSTOM_SOURCES}</Text><Pressable onPress={() => { if (customSourceLimitReached) { setMessage(`普通数据源已达到 ${MAX_CUSTOM_SOURCES} 个上限；官方 API 同步源不占用此上限。`); return; } setNewSourceName(""); setNewSourceAddress(""); setIsAddModalVisible(true); }} style={({ pressed }) => [styles.addSourceButton, pressed && styles.pressed]}><Text style={styles.addSourceButtonText}>＋ 添加数据源</Text></Pressable></View></View>
          ...
          {sources.length ? sources.map((source, index) => (
            <View key={source.id} style={[styles.sourceItem, endpoint?.apiUrl === source.id && styles.sourceItemActive]}>
              {editingSourceId === source.id ? (
                ...
                <View style={styles.sourceTop}>
                  <Pressable onPress={() => void switchSource(source.id)} style={({ pressed }) => [styles.sourceMain, pressed && styles.pressed]}>
                    ...
                    <View style={styles.sourceInfo}><View style={styles.sourceNameRow}><Text numberOfLines={1} style={styles.sourceName}>{source.displayName}</Text><Text style={source.sourceType === "official" ? styles.sourceOfficialTag : styles.sourceCustomTag}>{source.sourceType === "official" ? "官方" : "普通"}</Text></View><Text numberOfLines={1} style={styles.sourceAddress}>{source.endpoint.apiUrl}</Text><Text numberOfLines={1} style={[styles.sourceStatus, source.health === "unhealthy" && styles.sourceStatusUnhealthy, source.health === "unknown" && styles.sourceStatusUnknown]}>{source.health === "healthy" ? "数据正常" : source.health === "unhealthy" ? toChineseNetworkError(source.lastError, "数据验证失败，请稍后重试") : "尚未验证数据"}{source.lastCheckedAt ? ` · ${new Date(source.lastCheckedAt).toLocaleString("zh-CN")}` : ""}</Text></View>
                  </Pressable>
              ...
              <View style={styles.sourceActions}>
                ...
                <Pressable onPress={() => void runCheck(source.id)} disabled={checkingId === source.id} style={({ pressed }) => [styles.checkAction, (pressed || checkingId === source.id) && styles.pressed]}>{checkingId === source.id ? <ActivityIndicator color="#FFCD75" size="small" /> : <Text style={styles.checkActionText}>验证数据</Text>}</Pressable>
        ...
        <View style={styles.versionFooter}><Text style={styles.versionText}>fsvod-mobile-1.2.6</Text><Text style={styles.versionHint}>Android 独立侧载版</Text></View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  sourceHeading: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 4 },
  sourceHeaderActions: { flexDirection: "row", alignItems: "center", gap: 7, flexShrink: 0 },
  ...
  sourceCustomTag: { color: "#C5D2E5", backgroundColor: "#29364C", fontSize: 9, lineHeight: 14, fontWeight: "900", paddingHorizontal: 5, paddingVertical: 1, borderRadius: 4 },
