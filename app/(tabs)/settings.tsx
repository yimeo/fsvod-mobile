    ...
    <ScreenContainer containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          <View style={styles.sourceHeading}><View><Text style={styles.sectionTitle}>数据源管理</Text><Text style={styles.sourceIntro}>只有接口返回可解析的有效影视数据，才会显示为正常；仅能打开网址不算正常。</Text></View><View style={styles.sourceHeaderActions}><Text style={styles.sourceCount}>普通 {customSourceCount}/{MAX_CUSTOM_SOURCES} · 共 {sources.length}</Text><Pressable onPress={() => { if (customSourceLimitReached) { setMessage(`普通数据源已达到 ${MAX_CUSTOM_SOURCES} 个上限；官方 API 同步源不占用此上限。`); return; } setNewSourceName(""); setNewSourceAddress(""); setIsAddModalVisible(true); }} style={({ pressed }) => [styles.addSourceButton, customSourceLimitReached && styles.disabled, pressed && styles.pressed]}><Text style={styles.addSourceButtonText}>{customSourceLimitReached ? "已达上限" : "＋ 添加"}</Text></Pressable></View></View>
        ...
        <View style={styles.versionFooter}><Text style={styles.versionText}>fsvod-mobile-1.1.3</Text><Text style={styles.versionHint}>Android 独立侧载版</Text></View>
      </ScrollView>
