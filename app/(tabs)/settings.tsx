...
export default function SettingsScreen() {
  ...
  useEffect(() => subscribeTabRefresh("settings", () => {
    ...
    void loadCacheSummary();
    setMessage("正在刷新我的页面…");
    void syncOfficialResources(true).then(() => setMessage("我的页面已刷新。")).finally(() => setIsTabRefreshing(false));
    ...
    <ScreenContainer containerClassName="bg-background">
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.cacheHint}>影片播放线路和剧集信息会保存在设备中；海报使用磁盘缓存，显示实际占用大小。MP4、WebM 在线播放会使用原生缓存；iOS 因系统限制不能缓存在线 HLS（M3U8），如需离线播放请使用下载中心。</Text>
        </View>
